...
menu = ["Overview", 'Build Project', 'Prediction']
...
if choice == "Overview":
    ...
    st.write("Train test score: {}".format(round(train_score, 2)))
    st.write("Train test score: {}".format(round(train_score, 2)))
    st.write("Train test score: {}".format(round(train_score, 2)))
    st.table(confusion)
    ...
    if submit:
        ...
        if prediction[0] == 1:
            st.subheader('Passenger {} would have survived with a probability {}%'
            .format(name, round(predict_probability[0][1]*100, 2)))
        else:
            st.subheader('Passenger {} would not have survived with a probability {}%'
